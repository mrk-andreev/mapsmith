...
public final class TreeLongLongRangeMap implements LongLongRangeMap {
  ...
  private void removeClosed(long fromInclusive, long toInclusive) {
    Map.Entry<Long, Range> current = ranges.floorEntry(fromInclusive);
    if (current == null || current.getValue().to < fromInclusive) {
      current = ranges.ceilingEntry(fromInclusive);
    }

    while (current != null) {
      Range range = current.getValue();
      if (range.from > toInclusive) {
        return;
      }

      Map.Entry<Long, Range> next = ranges.higherEntry(range.from);
  ...
  private Range rangeFor(long key) {
    Map.Entry<Long, Range> entry = ranges.floorEntry(key);
    if (entry == null) {
      return null;
    }
  ...
  private void putClosed(long fromInclusive, long toInclusive, long value, boolean coalesce) {
    removeClosed(fromInclusive, toInclusive);
    Range storedRange = new Range(fromInclusive, toInclusive, value);
    if (coalesce) {
  ...
  private void putCoalescing(Range range) {
    Range merged = mergePrevious(range);
    merged = mergeNext(merged);
    ranges.put(merged.from, merged);
  }

  private Range mergePrevious(Range range) {
    Map.Entry<Long, Range> previousEntry = ranges.lowerEntry(range.from);
  ...
  private Range mergeNext(Range range) {
    Range result = range;
  ...
  private record Range(long from, long to, long value) {}
}
